...
DEFUALT_XAPI_STREAMING_PORT = 5113 # guess. Oryginal is 5125
...
class APIStreamClient(JsonSocket):
    def __init__(self, address=DEFAULT_XAPI_ADDRESS, port=DEFUALT_XAPI_STREAMING_PORT, encrypt=True, ssId=None, 
                 tickFun: Callable | None = None, tradeFun: Callable | None = None, balanceFun: Callable | None = None,
                 tradeStatusFun: Callable | None = None, profitFun: Callable | None = None, newsFun: Callable | None = None):
    ...
    def _readStream(self):
        while (self._running):
                ...
                if (msg["command"]=='tickPrices' and self._tickFun is not None):
                    self._tickFun(msg)
                elif (msg["command"]=='trade' and self._tradeFun is not None):
                    self._tradeFun(msg)
                elif (msg["command"]=="balance" and self._balanceFun is not None):
                    self._balanceFun(msg)
                elif (msg["command"]=="tradeStatus" and self._tradeStatusFun is not None):
                    self._tradeStatusFun(msg)
                elif (msg["command"]=="profit" and self._profitFun is not None):
                    self._profitFun(msg)
                elif (msg["command"]=="news" and self._newsFun is not None):
                    self._newsFun(msg)
